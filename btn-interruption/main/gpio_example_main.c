...
#define GET_OUTPUT_FROM_INPUT(val)  ((val == GPIO_INPUT_IO_0) ? GPIO_OUTPUT_IO_0 : GPIO_INPUT_IO_1)
...
static void gpio_task_example(void* arg)
{
    ...
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            ...
            uint8_t output_io = GET_OUTPUT_FROM_INPUT(io_num);
            gpio_set_level(output_io, !curr_level);
        }
    }
}
